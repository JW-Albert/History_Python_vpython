...
# 兩球高度時間圖
gd = graph(title = "y vs t" ,xtitle = "t" ,ytitle = "y" ,ymax = 13 ,xmax = 10 ,background = vector(0 ,0 ,0))
yt1 = gcurve(graph = gd ,color = color.yellow)
yt2 = gcurve(graph = gd ,color = color.red)
yt3 = gcurve(graph = gd ,color = color.blue)

# 3. 球的設定
ball3 = sphere(radius = 0.1 ,color = color.blue ,v = 0 ,m = 100)
ball1 = sphere(radius = 0.05 ,color = color.yellow ,v = 0 ,m = 0.2)
ball2 = sphere(radius = 0.1 ,color = color.red ,v = 0 ,m = 1)

ball3.pos = vector(0 ,2 + ball1.radius + ball2.radius + ball2.radius + 0.1 ,0)
...
while t <= 10:
    ...
    if mag(ball2.pos-ball3.pos) <= ball2.radius + ball3.radius :
